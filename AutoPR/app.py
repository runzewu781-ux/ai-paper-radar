...
from pragent.writing.blog_pipeline import generate_text_blog, generate_final_post, generate_wechat_post
from pragent.core.agents import setup_client, call_text_llm_api
from pragent.ai_denoise.post_clean import clean_khazix, l1_audit, md_to_html
...
async def process_pdf(
    ...
    try:
        ...
        progress(0.3, desc="Step 2/5: Reading figures with vision model (YOLO-free)...")
        ...
        vision_items = await run_figure_extraction_vision(
            ...
            api_key=vision_api_key, api_base=base_url, model=vision_model,
        )
        if not vision_items:
        ...
        final_post_md, assets_info = await generate_final_post(
            ...
            assets_dir=None,
            precomputed_items=vision_items,
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Accordion("Advanced Settings", open=True):
                ...
                _tmod = os.getenv("TEXT_MODEL", "qwen3.8-max-preview")
                _vmod = os.getenv("VISION_MODEL", "qwen3.8-max-preview")
                text_api_key_input = gr.Textbox(label="Text API Key", type="password", placeholder="Required: sk-...", value=_key)
                vision_api_key_input = gr.Textbox(label="Vision API Key (Optional)", type="password", placeholder="Optional: If not provided, Text API Key will be used", value=_key)
                base_url_input = gr.Textbox(label="API Base URL", value=_base)
